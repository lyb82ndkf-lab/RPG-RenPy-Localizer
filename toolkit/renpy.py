...
LIVE_FONT_OVERRIDE_NAME = "zz_rpgrtl_font_override.rpy"
LIVE_CJK_FONT_BASENAME = "RPGRenPyLocalizerCJK"
...
class RenPyService:
    ...
    def install_live_translation_bridge(self, clear_seen: bool = False) -> Path:
        ...
        # Live translation uses the game's running text styles directly, so it
        # needs the same bundled CJK fallback as exported translations.  This
        # is deliberately installed with the bridge instead of waiting for an
        # export/apply action that a live-only user may never run.
        self._install_live_font_override()
        try:
            (self.scripts_dir / LIVE_FONT_OVERRIDE_NAME).with_suffix(".rpyc").unlink()
        except FileNotFoundError:
            pass
        except OSError as exc:
            _append_live_log("tool", "font_override_rpyc_delete_failed", {"error": str(exc)})
    ...
    def clear_generated_translation_files(self) -> int:
        ...
        targets.extend(
            [
                ...
                self.scripts_dir / LIVE_FONT_OVERRIDE_NAME,
        ...
        for path in targets:
            try:
                ...
                continue
        # This font is copied exclusively by the live bridge.  Remove its
        # predictable generated names during restore so uninstalling the tool
        # leaves the game tree in the same state it was before live mode.
        for suffix in (".ttf", ".ttc", ".otf"):
            path = self.scripts_dir / "fonts" / f"{LIVE_CJK_FONT_BASENAME}{suffix}"
            try:
                if path.exists() and path.is_file():
                    path.unlink()
                    removed += 1
    ...
    def _install_live_font_override(self) -> Path | None:
        """Install a project-local CJK font and a late style override.

        A system path alone is fragile for packaged games and fails when the
        game is launched through a different working directory.  Copying the
        font below ``game/fonts`` gives Ren'Py a stable, relative resource path.
        """
        source = self._find_system_cjk_font()
        if source is None:
            return None
        fonts_dir = self.scripts_dir / "fonts"
        fonts_dir.mkdir(parents=True, exist_ok=True)
        suffix = source.suffix.lower() or ".ttf"
        destination = fonts_dir / f"{LIVE_CJK_FONT_BASENAME}{suffix}"
        try:
            shutil.copy2(source, destination)
            # Most Ren'Py builds accept a TTC through FreeType.  Keep the .ttf
            # alias as well because older projects commonly reference only TTF.
            alias = fonts_dir / f"{LIVE_CJK_FONT_BASENAME}.ttf"
            if destination != alias:
                shutil.copy2(source, alias)
        except OSError:
            return None

        output = self.scripts_dir / LIVE_FONT_OVERRIDE_NAME
        font_path = f"fonts/{LIVE_CJK_FONT_BASENAME}.ttf"
        lines = [
            "# Auto-generated by RPGRenPyLocalizer. Removes automatically on restore.",
            "init 999 python:",
            f"    _rpgrtl_live_cjk_font = {font_path!r}",
    ...
    def _is_generated_file(file_path: Path) -> bool:
        return file_path.name in {
            RUNTIME_PATCH_NAME,
            EMBEDDED_PATCH_NAME,
            LANGUAGE_BOOTSTRAP_NAME,
            EXTRACTOR_NAME,
            LIVE_BRIDGE_NAME,
            LIVE_FONT_OVERRIDE_NAME,
        }
